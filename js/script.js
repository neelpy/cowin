const secret =
  "U2FsdGVkX1+HQAr4dz+duq8zfH7xAvGNjw7bG1OGY9OvIuvNf3P+2J2J0iM1tu3ENPXB6/rPZsvqB+f54ikhuw==";
...
function showBeneficiaries() {
  ...
  $ben.text(
    eben.length > 0 ? eben.join(", ") : "Nobody eligible for this preference"
  );
}
...
async function check(centers) {
  ...
  const template = (center) => `
          <div class="p-1" style="border: 1px solid black">
              <b>${
                center.name
              }, Pincode: <a href="https://www.google.com/maps/place/${
    center["pincode"]
  }" target="_blank">${center["pincode"]}</a></b><br>
              ${center["sessions"]
                .filter((s) => 0 < s[cap])
                .map((s) => s["date"] + ": " + s[cap])
                .join("<br>")}<br>
  ...
  if (count === 0) {
    ...
    const alerting = new Audio();
    alerting.src = "assets/dingdong.mp3";
    alerting.play();
  }
...
async function setPreferences() {
  ...
  for (let i = 0; i < ben.length; i++) {
    ...
    const vaccineC =
      ...
      (dose === 2 &&
        (!vaccine ||
          vaccine === b["vaccine"].replace(/ /g, "-").toLowerCase()));
...
async function book() {
  ...
  try {
    ...
    console.log({ error });
    $bookingDiv.html(
      `<div id="booking-error" class="alert alert-danger">${error}</div>`
    );
